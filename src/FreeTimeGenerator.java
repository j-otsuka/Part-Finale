...
public class FreeTimeGenerator {
  ...
  void writeMeetingTimes(ArrayList<icsEvent> icsListA, ArrayList<icsEvent> icsListB){
    ...
    for (int i = 0; i < icsListA.size() || i < icsListB.size(); i++){
      //Use three if blocks to handle three cases: both lists have events left, only A has events left, or only B has events left.
      //System.out.println("Iterator count: " + i);
      if (i < icsListA.size() || i < icsListB.size()){
        //Store the current mapIndex as the start time of this free block.
        startTimes.add(mapIndex);
        //System.out.println("Both Map Index Start: " + mapIndex);
        
        //Count out the number of free minutes.
        while ((freeTimeMapA[mapIndex] != 1 && freeTimeMapB[mapIndex] != 1) && mapIndex < 1440){
          //System.out.println("freeTimeMapAStart: " + freeTimeMapA[mapIndex] + "\tfreeTimeMapBStart: " + freeTimeMapB[mapIndex]);
          freeTimeCounter++;
          mapIndex++;
        }
        ...
        while((freeTimeMapA[mapIndex] != 0 || freeTimeMapB[mapIndex] != 0) && mapIndex < 1440){
          //System.out.println("freeTimeMapAEnd: " + freeTimeMapA[mapIndex] + "\tfreeTimeMapBEnd: " + freeTimeMapB[mapIndex]);
          mapIndex++;
        }
        //System.out.println("Both Map Index End: " + mapIndex);
        startHour = (startTimes.get(i) / 60);
        startMinute = (startTimes.get(i) % 60);
